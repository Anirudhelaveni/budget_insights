...
export default function Dashboard() {
  ...
  const handleFileUpload = async (e) => {
    ...
    toast.promise(
      ...
      { loading: 'Uploading CSV...', success: 'Upload complete!', error: 'Upload failed.' }
    ).then(() => fetchExpenses());
  };
  ...
  const exportToCSV = () => {
    ...
    processedExpenses.forEach(exp => {
      ...
      csvRows.push([date, `"${exp.description.replace(/"/g, '""')}"`, exp.category || 'Other', parseFloat(exp.amount).toFixed(2)].join(','));
    });
  ...
  const navItemStyle = (tab) => ({ padding: '20px', cursor: 'pointer', backgroundColor: activeTab === tab ? '#1e293b' : 'transparent', borderLeft: activeTab === tab ? '4px solid #3b82f6' : '4px solid transparent' });

  return (
    <div style={containerStyle}>
      <Toaster position="bottom-right" />
      <aside style={{ width: '250px', backgroundColor: '#0f172a', color: 'white', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '30px 20px', fontSize: '20px', fontWeight: 'bold', borderBottom: '1px solid #334155' }}>💎 Budget Insights</div>
      ...
      <main style={{ flex: 1, padding: '40px', height: '100vh', overflowY: 'auto' }}>
        {activeTab === 'home' && <div style={cardStyle}><h1>Welcome to Budget Insights! 👋</h1></div>}
        {activeTab === 'support' && <div style={cardStyle}><h1>Need Help?</h1><p>Contact: support@budgetinsights.com</p></div>}
        {activeTab === 'dashboard' && (
          <>
            <div style={{ display: 'flex', gap: '20px', marginBottom: '30px' }}>
              <div style={cardStyle}>Total Spent: ${totalSpent.toFixed(2)}</div>
              <div style={cardStyle}>Highest Category: {highestCategory}</div>
              <div style={cardStyle}>Average: ${avgExpense.toFixed(2)}</div>
            </div>
            <div style={cardStyle}>
              ...
              <ResponsiveContainer width="100%" height={300}>
                {viewMode === 'pie' ? (
                  <PieChart><Pie data={chartData} dataKey="value" nameKey="name" label>{chartData.map((e,i) => <Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip /><Legend /></PieChart>
                ) : (
